...
$(document).ready(function(){
    ...
    $('#btnAgregar').click(function(){
        $('#inputTitle').html("Agregar Lista de Valores");
    ...
    $(document).on('click','.btn-editar',function(){
        ...
        $('#inputTitle').html("Editar Lista de Valores");
        $.get(url + '/' + ide_item, function (data) {
            //success data
            console.log(data);
            $('#inGrupoLista').val(data.grupo_lista);
            $('#inCodigoLista').val(data.codigo_lista);
            $('#inDescripcion').val(data.descripcion);
            $('#btnGuardar').val('update');
            $('#agregarEditarModal').modal('show');
            $('#ide_item').val(data.ide_lista);
            $('#loading').modal('hide');
        }) 
    });    
    ...
    $("#btnGuardar").click(function (e) {      
        var formData = {
            grupo_lista: $('#inGrupoLista').val(),
            codigo_lista: $('#inCodigoLista').val(),
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data); 
                var item = '<tr class="even gradeA" id="item'+data.ide_lista+'">'
                    item+='<td>'+data.grupo_lista+'</td>'
                    item+='<td>'+data.codigo_lista+'</td>'
                    item+='<td>'+data.descripcion+'</td>';
                    item+='<td><button class="btn btn-primary btn-editar" value="'+data.ide_lista+'"><i class="icon-pencil icon-white" ></i> Editar</button>';
                    item+='<button class="btn btn-danger" value="'+data.ide_lista+'"><i class="icon-remove icon-white"></i> Eliminar</button></td></tr>';
                if (state == "add"){ 
            ...
            error: function (data) {
                $('#loading').modal('hide');
                var errHTML="";
                if(data.responseJSON.hasOwnProperty("codigo_lista")){
