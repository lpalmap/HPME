...
class HPMEConstants
{
    ...
    const PLN_METAS_NO_INGRESADASV2="SELECT distinct m.nombre FROM pln_producto_indicador p,pln_proyecto_meta pm,cfg_meta m,pln_proyecto_region pr WHERE pm.ide_proyecto=p.ide_proyecto AND pm.ide_meta=p.ide_meta AND pm.ind_obligatorio='S' AND p.ide_meta=m.ide_meta  and pr.ide_proyecto_planificacion=pm.ide_proyecto and pr.ide_proyecto_region=:ideProyectoRegion AND NOT EXISTS(SELECT r.ide_region_producto from pln_region_producto r WHERE r.ide_producto_indicador=p.ide_producto_indicador AND r.ide_proyecto_region=:ideProyectoRegion)";
