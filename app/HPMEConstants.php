...
class HPMEConstants
{
    ...
    const PLN_METAS_POR_PROYECTO="SELECT p.ide_proyecto_meta,m.nombre FROM pln_proyecto_meta p,cfg_meta m WHERE p.ide_proyecto=:ideProyecto and p.ide_meta=m.ide_meta order by m.orden";
    const PLN_OBJETIVOS_POR_META="SELECT p.ide_objetivo_meta,o.nombre FROM pln_objetivo_meta p,cfg_objetivo o WHERE o.ide_objetivo=p.ide_objetivo and p.ide_proyecto_meta=:ideProyectoMeta ORDER BY o.orden";
    const PLN_AREAS_POR_OBJETIVO="SELECT p.ide_area_objetivo,a.nombre,a.orden_especial FROM pln_area_objetivo p,cfg_area_atencion a where a.ide_area=p.ide_area and p.ide_objetivo_meta=:ideObjetivoMeta ORDER BY a.orden";
    const PLN_INDICADORES_POR_AREA="SELECT p.ide_indicador_area,i.nombre FROM pln_indicador_area p,cfg_indicador i WHERE i.ide_indicador=p.ide_indicador AND p.ide_area_objetivo=:ideAreaObjetivo ORDER BY i.orden";
    const PLN_PRODUCTOS_POR_INDICADOR="SELECT p.ide_producto_indicador,c.nombre,c.orden,c.requiere_comprobantes  FROM pln_producto_indicador p,cfg_producto c WHERE c.ide_producto=p.ide_producto AND p.ide_indicador_area=:ideIndicadorArea ORDER BY c.orden";
    const PLN_REGION_PRODUCTO="SELECT r.ide_region_producto,p.nombre as proyecto,r.descripcion FROM pln_region_producto r left join cfg_proyecto p on r.ide_proyecto=p.ide_proyecto WHERE r.ide_proyecto_region=:ideProyectoRegion AND r.ide_producto_indicador=:ideProductoIndicador";    
    const PLN_DETALLE_POR_PRODUCTO_REGION="SELECT d.ide_region_producto_detalle,d.num_detalle,sum(d.valor) as valor,sum(d.ejecutado) as ejecutado FROM pln_region_producto r,pln_region_producto_detalle d WHERE d.ide_region_producto=r.ide_region_producto and r.ide_region_producto=:ideRegionProducto GROUP BY d.num_detalle ORDER BY d.num_detalle";
    const PLN_DETALLE_POR_PRODUCTO_REGION_PERIODO="SELECT d.ide_region_producto_detalle,d.num_detalle,sum(d.valor) as valor,sum(d.ejecutado) as ejecutado FROM pln_region_producto r,pln_region_producto_detalle d WHERE d.ide_region_producto=r.ide_region_producto and r.ide_region_producto=:ideRegionProducto and d.num_detalle=:periodo GROUP BY d.num_detalle";
    const PLN_CONSOLIDADO_POR_PRODUCTO="SELECT d.num_detalle,sum(d.valor) as valor,sum(d.ejecutado) as ejecutado FROM pln_region_producto r,pln_region_producto_detalle d WHERE d.ide_region_producto=r.ide_region_producto and r.ide_producto_indicador=:ideProductoIndicador GROUP BY d.num_detalle ORDER BY d.num_detalle";
    const PLN_USUARIOS_BITACORA_PLANIFICACION="SELECT distinct u.email FROM pln_bitacora_mensaje b,seg_usuario u WHERE b.ide_bitacora_proyecto_region=:ideBitacora AND b.ide_usuario=u.ide_usuario AND b.ide_usuario<>:myUsuario";
    const PLN_OBSERVACIONES_EMAIL_ADMINISTRADOR="SELECT u.email FROM seg_usuario_region r,seg_usuario u WHERE r.ide_usuario=u.ide_usuario AND r.ide_region=:ideRegion";
    const PLN_NOMBRE_PRODUCTO_REGION="select p.nombre from pln_region_producto r,pln_producto_indicador i,cfg_producto p where r.ide_producto_indicador=i.ide_producto_indicador and i.ide_producto=p.ide_producto and r.ide_region_producto=:ideProductoRegion";
    ...
    const PLN_METAS_NO_INGRESADASV2="SELECT distinct m.nombre FROM pln_producto_indicador p,pln_proyecto_meta pm,cfg_meta m,pln_proyecto_region pr WHERE pm.ide_proyecto=p.ide_proyecto AND pm.ide_meta=p.ide_meta AND pm.ind_obligatorio='S' AND p.ide_meta=m.ide_meta  and pr.ide_proyecto_planificacion=pm.ide_proyecto and pr.ide_proyecto_region=:ideProyectoRegion1 AND NOT EXISTS(SELECT r.ide_region_producto from pln_region_producto r WHERE r.ide_producto_indicador=p.ide_producto_indicador AND r.ide_proyecto_region=:ideProyectoRegion2)";
    ...
    const PLN_PRESUPUESTO_POR_DEPARTAMENTO_USUARIO="SELECT p.ide_presupuesto_departamento,d.nombre,p.fecha_ingreso,p.fecha_aprobacion,p.estado,u.usuario FROM pln_presupuesto_departamento p left join seg_usuario u on p.ide_usuario_aprobacion=u.ide_usuario join cfg_departamento d on p.ide_departamento=d.ide_departamento WHERE p.ide_proyecto_presupuesto=:ideProyectoPresupuesto AND d.ide_usuario_director=:ideUsuario";
    ...
    const PLN_PRESUPUESTO_DETALLE_COLABORADOR="SELECT c.nombres,c.apellidos,p.ide_presupuesto_departamento,d.ide_proyecto_presupuesto,d.ide_departamento FROM pln_presupuesto_colaborador p,cfg_colaborador_proyecto c,pln_presupuesto_departamento d WHERE p.ide_colaborador=c.ide_colaborador AND p.ide_presupuesto_departamento=d.ide_presupuesto_departamento AND p.ide_presupuesto_colaborador=:idePresupuestoColaborador";
    ...
    const PLN_OBSERVACIONES_PRESUPUESTO_EMAIL_ADMINISTRADOR="SELECT u.email FROM cfg_departamento d,seg_usuario u WHERE d.ide_usuario_director=u.ide_usuario AND d.ide_departamento=:ideDepartamento";
  ...
  where d.ide_colaborador_cuenta=p.ide_colaborador_cuenta and p.ide_presupuesto_colaborador=l.ide_presupuesto_colaborador and p.ide_cuenta=c.ide_cuenta AND c.ide_cuenta_padre=:ideCuentaPadre AND l.ide_presupuesto_departamento=:idePresupuestoDepartamento GROUP BY c.ide_cuenta,c.cuenta,c.nombre";
    ...
    const PARAM_EMAIL_PLANIFICACION='EMAIL_PLANIFICACION';
    const PARAM_EMAIL_PRESUPUESTO='EMAIL_PRESUPUESTO';
    const FILTRO_TODO=0;
