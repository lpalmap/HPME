                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <button class="btn btn-success" id="btnAgregar"><i class="icon-plus icon-white" ></i> Agregar Lista</button>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="dataTableItems">
                                    ...
                                    <tbody id="lista-items" name="lista-items">
                                        ...
                                        <tr class="even gradeA" id="item{{$items[$i]->ide_lista}}">
                                            <td>{{$items[$i]->grupo_lista}}</td>
                                            <td>{{$items[$i]->codigo_lista}}</td>
                                            <td>{{$items[$i]->descripcion}}</td>
                                            <td>
                                                <button class="btn btn-primary btn-editar" value="{{$items[$i]->ide_lista}}"><i class="icon-pencil icon-white" ></i> Editar</button>
                                                <button class="btn btn-danger" value="{{$items[$i]->ide_lista}}"><i class="icon-remove icon-white"></i> Eliminar</button>
                                            </td>
                                        </tr>
                                            @endfor
                                        @endif
                                    ...
                                    <div class="modal-content">
                                       ...
                                       <form role="form" id="formAgregar">
                                           <div class="form-group">
                                               <label>Grupo Lista</label>
                                               <select id="inGrupoLista" class="form-control">
                                                   @for ($i=0;$i<count($grupos);$i++)
                                                       <option value="{{$grupos[$i]}}">{{$grupos[$i]}}</option>
                                                    @endfor
                                               </select>
                                           </div>
                                           <div class="form-group">
                                               <label>C&oacute;digo</label>
                                                <input class="form-control" id="inCodigoLista" required="true"/>
                                            </div>
                                            <div class="form-group">
...
@section('footer')
    @parent
        ...
        <script src="{{asset('js/hpme.listas.js')}}"></script>
@endsection